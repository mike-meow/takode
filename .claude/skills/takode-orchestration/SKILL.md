...
- **Before dispatching any quest, *ALWAYS* invoke `/leader-dispatch`.** It owns worker selection, the initial Journey proposal, and the planning-only dispatch template. Get approval for the planned initial Journey there before spawning a worker. Do not add extra context, file paths, or investigation instructions to the dispatch message -- add any extra information into the quest itself before dispatching.
