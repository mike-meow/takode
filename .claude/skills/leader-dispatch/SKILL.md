...
- **Treat worker/reviewer confusion as a quest-scoped blocking signal.** When a herded worker or reviewer raises a clarification question, answer it from existing context if you can. If not, ask the user in plain text first, then call `takode notify needs-input` with a short summary, keep that specific quest blocked until the ambiguity is resolved, and continue unrelated orchestration. If you intentionally pause the quest on human input, make that an explicit board decision rather than inferring the pause from `/confirm` alone.
