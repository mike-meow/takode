...
- [ ] **Fix session creation from CLI/assistant missing name and model** — Sessions created via `companion sessions create` or the Companion assistant now get a server-generated name and default model. Verify: run `companion sessions create --port 3457 --cwd /home/jiayiwei/companion`, confirm the session appears in the sidebar with a name like "Swift Falcon". Also verify the assistant creates sessions that appear in the sidebar.
